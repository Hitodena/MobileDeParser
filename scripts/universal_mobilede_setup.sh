...
start_container() {
    ...
    if sudo docker ps -a | grep -q -E " $CONTAINER_NAME$"; then
    ...
    if [ $? -eq 0 ]; then
        ...
        sleep 10
        if sudo docker ps | grep -q -E " $CONTAINER_NAME$"; then
            print_status "Контейнер работает корректно"
            sudo docker ps | grep -E " $CONTAINER_NAME$"
        else
...
show_status() {
    ...
    if sudo docker ps | grep -q -E " $CONTAINER_NAME$"; then
        ...
        if sudo docker ps -a | grep -q -E " $CONTAINER_NAME$"; then
            ...
            sudo docker ps -a | grep -E " $CONTAINER_NAME$"
        fi
    fi
...
check_system() {
    ...
    if sudo docker ps -a | grep -q -E " $CONTAINER_NAME$"; then
        ...
        sudo docker ps -a | grep -E " $CONTAINER_NAME$" | while read line; do
            echo "  $line"
        done
    else
